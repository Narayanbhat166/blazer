...
pub async fn room_service(
    ...
    let request_type = types::RoomServiceRequestType::from_u8(request.request_type as u8).ok_or(
    ...
    match request_type {
        types::RoomServiceRequestType::CreateRoom => {
        ...
        types::RoomServiceRequestType::JoinRoom => {
            ...
            let (response, should_close_stream) = match item {
                types::Message::GameStart { room_id, users } => (
                    Some(RoomServiceResponse {
                        ...
                        message_type: types::RoomServiceResponseType::GameStart.to_u8().into(),
                ...
                types::Message::RoomCreated { room_id, users } => (
                    Some(RoomServiceResponse {
                        room_id: Some(room_id),
                        message_type: types::RoomServiceResponseType::Init.to_u8().into(),
                        user_details: users.into_iter().map(Into::into).collect::<Vec<_>>(),
                    }),
                    false,
                ),
                ...
                types::Message::UserJoined { room_id, users } => (
                    Some(RoomServiceResponse {
                        room_id: Some(room_id),
                        message_type: types::RoomServiceResponseType::UserJoined.to_u8().into(),
                        user_details: users.into_iter().map(Into::into).collect::<Vec<_>>(),
                    }),
                    false,
                ),
